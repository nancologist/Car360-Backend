...
public class ColorImageWriter {
    ...
    public static void main(String[] args) throws SQLException {
        try {
            ...
            try (PreparedStatement updateQueryPS = connection.prepareStatement(
                    updateQuery); PreparedStatement importQueryPS = connection.prepareStatement(importQuery)) {
                ...
                for (File file : Objects.requireNonNull(imgDir.listFiles())) {
                    if (file.isFile()) {
                        ...
                        String colorCode = Arrays.stream(file.getName().split("-"))
                                                 .map(item -> item.split("\\.")[0])
                                                 .reduce((a, b) -> b)
                                                 .orElseThrow(() -> new IllegalStateException("Color code not found"));
                        ...
                        // String unlinkQuery = "SELECT lo_unlink(?)"; // For clean up the database // Todo: implement
                        // this.
                    }
                }
